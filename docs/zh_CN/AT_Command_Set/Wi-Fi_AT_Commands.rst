...
.. only:: esp32c2
  ...
  .. important::
    默认的 {IDF_TARGET_NAME} 4MB AT 固件支持此页面下的所有 AT 命令。默认的 {IDF_TARGET_NAME} 2MB AT 固件支持此页面下除 :ref:`AT+CWJEAP <cmd-JEAP>` 之外的所有 AT 命令。如果你需要修改 {IDF_TARGET_NAME} 默认支持的命令，请自行 :doc:`编译 ESP-AT 工程 <../Compile_and_Develop/How_to_clone_project_and_compile_it>`，在第五步配置工程里选择（下面每项是独立的，根据你的需要选择）：

    - 启用或禁用 EAP 命令（:ref:`AT+CWJEAP <cmd-JEAP>`）： ``Component config`` > ``AT`` > ``AT Wi-Fi Enterprise command support``
