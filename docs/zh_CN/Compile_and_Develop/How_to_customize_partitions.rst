...
{IDF_TARGET_AT_SECOND_PARTITION_ADDR: default="undefined", esp32="0x20000", esp32c2="0x1E000", esp32c3="0x1E000", esp32c5="0x30000", esp32c6="0x1E000", esp32c61="0x30000", esp32s2="0x20000"}
...
ESP-AT 提供了二级分区表 at_customize.csv 供你存储自定义数据块。它基于一级分区表。
...
.. only:: esp32c5
  ...
  .. list-table:: at_customize.csv 路径
    ...
    * - 平台
      ...
      - 路径
    * - ESP32-C5
      - ESP32C5-4MB（所有带 4 MB flash 的 ESP32-C5 系列）
...
.. only:: esp32c61
  ...
  .. list-table:: at_customize.csv 路径
    ...
    * - 平台
      ...
      - 路径
    * - ESP32-C61
      - ESP32C61-4MB（所有带 4 MB flash 的 ESP32-C61 系列）
      - :project_file:`module_config/module_esp32c61_default/at_customize.csv`
...
- 如果你需要添加一个新的用户分区，请先检查它是否已经在 ESP-IDF (``esp_partition.h``) 中定义。
...
修改 at_customize.csv 后，你可以重新编译 ESP-AT 工程或使用 python 脚本 ``gen_esp32part.py`` 来生成 at_customize.bin 文件。
...
.. only:: esp32c6
  ...
  .. list-table:: 不同模组 at_customize.bin 的下载地址
    ...
    * - 平台
      ...
      - 大小
    * - {IDF_TARGET_NAME}
      - {IDF_TARGET_CFG_PREFIX}-4MB（所有带 4 MB flash 的 {IDF_TARGET_NAME} 系列）
      - 0x1E000
      - 0x42000
